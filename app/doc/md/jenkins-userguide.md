...
```
# docker build -t jaeminjeo/my-jenkins:1.0 .
```
...
```
# docker login
# docker push jaeminjeo/my-jenkins:1.0
```

컨테이너 생성 및 실행하기

```
# docker run --privileged -d -v /var/run/docker.sock:/var/run/docker.sock -v jenkins_home:/var/jenkins_home -p 8080:8080 -p 50000:50000 --restart=on-failure --network="jenkins" --name my-jenkins jaeminjeo/my-jenkins:1.0
# docker container ls
```
...
- 설정
  - General
    - 설명: `빌드 테스트1`
    - `GitHub project` 체크
      - Project url: `https://github.com/minjaejeo/ncp-myapp.git`
  - 소스 코드 관리
    - `Git` 선택
      - Repository URL: `https://github.com/minjaejeo/ncp-myapp.git`
      - Credentials:
